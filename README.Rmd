...
- **Shared transport base via [github.com/dereckscompany/connectcore](https://github.com/dereckscompany/connectcore).** `HyperliquidBase` inherits `connectcore::RestClient` for credential storage, the sync/async perform function, and the overridable error seam (Hyperliquid's two-failure-shape parser plugs into `.parse_envelope()`). Because Hyperliquid signs the request **body** (not the request) and posts the exact signed JSON on the wire, the funnel itself stays Hyperliquid-specific.
