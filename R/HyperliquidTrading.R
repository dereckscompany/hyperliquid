...
HyperliquidTrading <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param is_buy Logical; `TRUE` for a bid, `FALSE` for an ask.
    #' @param sz scalar<numeric in ]0, Inf[>; the order size in coin units.
    #' @param limit_px scalar<numeric in ]0, Inf[>; the limit price.
    #' @param order_type Named list; either
    #'   `list(limit = list(tif = "Gtc"|"Ioc"|"Alo"))` or
    #'   `list(trigger = list(triggerPx = , isMarket = , tpsl = "tp"|"sl"))`.
    #' @param reduce_only Logical; `TRUE` to only reduce an existing position.
    #'   Default `FALSE`.
    #' @param cloid Character or `NULL`; an optional client order id from
    ...
    #'   Default `NULL`.
    #' @param grouping Character; one of `"na"`, `"normalTpsl"`, `"positionTpsl"`.
    #'   Default `"na"`.
    #' @return A [data.table::data.table], one row per status, or a promise
    #'   thereof.
    bulk_orders = function(orders, builder = NULL, grouping = "na") {
      ...
      return(private$.submit_l1(action, .parser = parse_order_response))
    },
    ...
    #' @param name Character; the coin or friendly name, e.g. `"BTC"`.
    #' @param is_buy Logical; `TRUE` to open long, `FALSE` to open short.
    #' @param sz scalar<numeric in ]0, Inf[>; the order size in coin units.
    #' @param slippage scalar<numeric in ]0, Inf[>; the price tolerance fraction.
    #'   Default `0.05` (5%).
    #' @param cloid Character or `NULL`; an optional client order id. Default
    #'   `NULL`.
    #' @param builder Named list or `NULL`; an optional builder fee spec. Default
    #'   `NULL`.
    #' @return A [data.table::data.table], one row per status.
    market_open = function(name, is_buy, sz, slippage = 0.05, cloid = NULL, builder = NULL) {
      validate_coin(name)
      assert::assert_scalar_logical(is_buy)
      assert_finite_positive(sz, "sz")
      assert_finite_positive(slippage, "slippage")
    ...
    #' @param name Character; the coin or friendly name, e.g. `"BTC"`.
    #' @param sz scalar<numeric in ]0, Inf[> or `NULL`; the size to close.
    #'   `NULL` (default) closes the whole position (`abs(szi)`).
    #' @param slippage scalar<numeric in ]0, Inf[>; the price tolerance fraction.
    #'   Default `0.05` (5%).
    #' @param cloid Character or `NULL`; an optional client order id. Default
    #'   `NULL`.
    ...
    #' @param cancels Unnamed list of cancel specs; each a named list with `coin`
    #'   and `oid`.
    #' @return A [data.table::data.table], one row per cancel, or a promise
    #'   thereof.
    bulk_cancel = function(cancels) {
      ...
      return(private$.submit_l1(action, .parser = parse_cancel_response))
    },
    ...
    #'   day.
    #' @param time POSIXct, numeric epoch-milliseconds, or `NULL`; when to cancel
    #'   all orders. `NULL` (default) clears any pending schedule.
    ...
    schedule_cancel = function(time = NULL) {
      ...
      return(private$.submit_l1(action, .parser = parse_action_status))
    },

    #' @description Set the leverage for a coin, cross or isolated.
    #' @param name Character; the coin or friendly name.
    #' @param leverage Numeric; a finite, strictly-positive whole number.
    #' @param is_cross Logical; `TRUE` for cross margin, `FALSE` for isolated.
    #'   Default `TRUE`.
    #' @return A single-row [data.table::data.table] with `status` and
    #'   `response_type`, or a promise thereof.
    #' @importFrom rlang abort
    update_leverage = function(name, leverage, is_cross = TRUE) {
      validate_coin(name)
      assert::assert_scalar_numeric(leverage)
      if (!is.finite(leverage) || leverage <= 0 || leverage != trunc(leverage)) {
        rlang::abort(sprintf(
          "`leverage` must be a finite, strictly-positive whole number, got: %s",
          format(leverage)
        ))
      }
      assert::assert_scalar_logical(is_cross)
      action <- list(
        type = "updateLeverage",
    ...
    #' @param name Character or `NULL`; an optional human-readable agent name.
    #'   Default `NULL`.
    #' @return A single-row [data.table::data.table] with `agent_address`,
    #'   `agent_key` (the new hex secret), and `status`, or a promise thereof.
    #' @importFrom openssl rand_bytes
    approve_agent = function(name = NULL) {
      assert::assert_scalar_character(name, null_ok = TRUE)
    ...
    #' @param builder Character; the builder's `0x`-prefixed address.
    #' @param max_fee_rate Character; the max fee as a percent string, e.g.
    #'   `"0.001%"`.
    #' @return A single-row [data.table::data.table] with `status` and
    #'   `response_type`, or a promise thereof.
    approve_builder_fee = function(builder, max_fee_rate) {
      builder <- validate_address(builder)
      assert::assert_scalar_character(max_fee_rate)
      action <- list(
