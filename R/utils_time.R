...
#'
#' @param x (POSIXct | Date | numeric?) a POSIXct, Date, or value coercible by
#'   [lubridate::as_datetime()] (`NULL` passes through).
...
datetime_to_ms <- function(x) {
  ...
  return(assert_return_datetime_to_ms(
    floor(as.numeric(lubridate::as_datetime(x, tz = "UTC")) * 1000)
  ))
}
...
#' @return (POSIXct) a vector in UTC.
#'
#' @examples
...
time_convert_to_hyperliquid <- function(datetime) {
  assert_args_time_convert_to_hyperliquid(datetime)
  return(assert_return_time_convert_to_hyperliquid(datetime_to_ms(datetime)))
}
