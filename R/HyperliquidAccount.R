...
HyperliquidAccount <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param address (scalar<character>) the account's `0x`-prefixed address.
    #'   Defaults to the instance's acting address.
    #' @param order_id_or_client_order_id (scalar<numeric> | scalar<character>)
    #'   numeric order id, or character `0x`-prefixed client order id.
    #' @return (promise<data.table>) a single-row [data.table::data.table] (or a
    ...
    get_order_status = function(address = private$.acting_address(), order_id_or_client_order_id) {
      assert_args_HyperliquidAccount__get_order_status(address, order_id_or_client_order_id)
      address <- validate_address(address)
      return(private$.info(
        list(type = "orderStatus", user = address, oid = order_id_or_client_order_id),
