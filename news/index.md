...
**The README now follows the one shape used across the whole connector
fleet, and it gains a citation and a map of the vignettes.** This
release reshapes `README.Rmd` into the fleet’s canonical section order
(owner ruling 20, 2026-09-18): a plain-English lead, a technical
overview, a design philosophy, installation, authentication and the rest
of the per-surface sections, asynchronous usage, and finally a
documentation index, a citation, and the licence. No sentence was
rewritten and no code chunk’s behaviour changed; existing sections were
only renamed or moved into their new slots, and two sections that did
not exist before, Documentation and Citation, were built from the
package’s own metadata.

- Renamed `## Design` to `## Design philosophy` and
  `## Asynchronous Use` to `## Asynchronous usage`; the prose beneath
  both is unchanged.
- `## Quick start` is omitted: `## Authentication` keeps its original
  name in the per-surface group, since its content is wallet-key
  authentication rather than a walkthrough, and the canonical shape only
  requires Quick start where a repository genuinely has one.
- Moved `## Disclaimer` from between Technical overview and Design into
  the usage-surface group, now immediately after Authentication and
  before Market Data; moved `## Available Classes` from after
  Asynchronous Use to the end of the same group, now immediately before
  Asynchronous usage.
- Merged `## Author` into the new `## Citation` section: same person,
  same ORCID link, no longer a separate heading.
- Added `## Documentation`, naming the pkgdown site (the first,
  github.io URL in `DESCRIPTION`) and the three vignettes in their
  `_pkgdown.yml` reading order (getting-started, trading-strategies,
  live-testnet-walkthrough), each with a one-line description, plus a
  pointer to `NEWS.md`.
- Added `## Citation`, a “Cite as” line built only from `DESCRIPTION`
  (`Authors@R`, `Title`, `Version`, and the repository URL) with the
  year taken from this entry; no DOI or journal, since neither exists.
- `## Error handling` was omitted: the package has no dedicated
  error-handling documentation to move, only a passing mention of the
  error seam already kept under Design philosophy.
- `README.md` re-rendered from `README.Rmd` against the shared mock
  router; no chunk options, `eval` flags, or fixtures changed.
...
(e.g. `"2976574.9037540001"`). Worse, the R-list fixtures that back the
parser unit tests were never scrubbed at all: they still carried the
real 40-hex addresses of the HLP vaults, a retail account, and a
validator, plus thirteen real 64-hex on-chain transaction hashes — the
same underlying capture the JSON fixtures were quietly built from,
confirmed by lining every placeholder back up against the real value it
stood in for. None of that belongs in a public repository, however the
addresses were described as “scrubbed”. The fleet rule (ratified
2026-07-05, re-ratified 2026-09-17) is that fixtures are hand-authored
and synthetic from the start — never captured, never
scrubbed-and-shipped. This release brings hyperliquid into line.

- Rewrote every real address and on-chain hash still present in
  `tests/testthat/fixtures-account.R`,
  `tests/testthat/fixtures-marketdata.R`, and
  `tests/testthat/fixtures-staking.R` (plus two `@examples` blocks in
  `R/HyperliquidAccount.R` and `R/HyperliquidStaking.R`) to the same
  patterned placeholder scheme already used in
  `tests/testthat/fixtures/*.json` (`0x000...000N` for a user/vault
  address, `0x000...0010` for the validator, `0x` + 62 zeros + a
  two-digit suffix for a hash) — each real value was matched to the
  placeholder already standing in for it in the JSON fixture built from
  the same capture, so both fixture paths now agree.
- Replaced every IEEE-float-tailed balance across both fixture corpora
  (account value, margin, withdrawable, liquidation price, portfolio
  history, sub-account equity, vault equity, ledger amounts, fee
  volumes, rate-limit cumulative volume, staking delegation/reward
  amounts, and asset-context day volume/supply figures) with round
  authored numbers, updating the handful of test assertions in
  `test-account.R`, `test-marketdata.R`, and `test-staking.R` that
  checked an exact old value. No row count, column set, or column
  contract changed.
- Corrected `tests/testthat/mock_router.R`’s header, which had claimed
  fixtures were “the REAL captured … Hyperliquid JSON” only
  address/hash-scrubbed: it now states plainly that every fixture is
  authored synthetic data, shape-faithful to Hyperliquid’s documented
  responses but never captured from a live account.
  `fixtures-account.R`, `fixtures-marketdata.R`, and
  `fixtures-staking.R`’s own headers (which named the HLP child vault,
  the HLP parent vault, and “a retail address” the fixtures were
  supposedly captured against) are corrected the same way, and matching
  “captured fixture” wording in `test-account.R`, `test-marketdata.R`,
  `test-staking.R`, and `test-parse-precision.R` is corrected too.
  NEWS.md’s 0.2.1 entry, which announced the capture-based
  fixture-validation practice this release retires, is left as written
  for the historical record with a note appended underneath pointing
  here.
- `test-signing.R`’s pinned `{r,s,v}` signature vectors and the
  well-known python/rust-SDK test private key and destination address
  they reuse (also referenced by a couple of other tests as an input
  literal) are deliberately synthetic reference vectors, not a capture,
  and are untouched. `fixtures-trading.R` and `fixtures-transfers.R`
  were already hand-authored synthetic data and are untouched.
- README.md re-rendered from README.Rmd, every vignette re-knit, and
  `man/` regenerated against the corrected fixtures and roxygen
  examples; no behaviour or column-contract change anywhere in `R/`.
...
- Added `tests/testthat/test-parse-precision.R`: drives `get_candles()`,
  `get_meta_and_asset_ctxs()`, and `get_funding_history()` through the
  real public client, via a synthetic high-precision fixture routed
  through the shared `connectcore` mock harness (`body_routes()` +
  `local_mock_api()`). Every numeric column is asserted
  `expect_identical()` (never tolerance-based) against
  [`as.numeric()`](https://rdrr.io/r/base/numeric.html) of the fixture’s
  own decimal string, and a big-integer-looking identifier column is
  asserted to stay character and unchanged.
- No behaviour change: the parse path (`num_or_na()` -\>
...
- [`get_api_keys()`](https://dereckscompany.github.io/hyperliquid/reference/get_api_keys.md)
  ...
  return all-`NULL`), and the warning now says which mode you are in:
  READ-ONLY (address kept, signing impossible) versus public-only
  (neither set).
- `HyperliquidAccount`’s acting-address resolution is unchanged —
  `account_address` already took precedence — so read-only credentials
  flow through every `/info` account read with no other change.
...
- Added `dev/capture-hyperliquid.R`, a read-only capture harness that
  ...
  NOTE (added later, see the 0.7.3 entry): this pass validated fixture
  *shape* against a live capture, but some of the resulting committed
  fixtures went further and kept real captured account data (addresses,
  on-chain hashes, and account balances) rather than authoring synthetic
  values from that shape. 0.7.3 rewrites every such fixture as authored
  synthetic data; `dev/capture-hyperliquid.R` and
  `data-raw/capture_testnet_fixtures.R` remain in the repository as
  read-only shape-validation tools whose own output is git-ignored.
