...
test_that("HyperliquidAccount public methods round-trip through the router", {
  ...
  expect_true(data.table::is.data.table(account$get_order_status(.addr, order_id_or_client_order_id = 461291857943)))
...
test_that("HyperliquidTrading public methods round-trip through the router", {
  ...
  expect_true(data.table::is.data.table(trading$market_open("BTC", is_buy = TRUE, size = 0.001)))
  ...
  expect_true(data.table::is.data.table(trading$cancel_order("BTC", order_id = 123)))
  expect_true(data.table::is.data.table(trading$bulk_cancel(list(list(coin = "BTC", oid = 123)))))
  expect_true(data.table::is.data.table(trading$cancel_by_cloid("BTC", client_order_id = new_cloid())))
