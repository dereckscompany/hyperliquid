...
## Convention: reusable `empty_dt_*` typed-empty constructors

* Adopted the sibling connectors' `empty_dt_*` convention package-wide. The zero-row schemas were inlined at each parser's empty branch; they are now defined once as reusable `empty_dt_<descriptive>()` constructors in `R/helpers_parse.R` (37 in total, one per endpoint shape), and every endpoint parser's empty branch calls the matching constructor instead of building (or omitting) the schema inline. This also extends the typed-empty guarantee to the endpoints whose `@return` contract did not strictly require columns (`get_spot_balances()`, `get_open_orders()`, `get_recent_trades()`, the portfolio / fees / sub-account / staking reads, …): an empty live response now returns a stable, fully-typed zero-row table everywhere, not a column-less one. The constructors close with `[]` so the returned table prints on the first call, and timestamp columns use the same `ms_to_datetime()` coercion the parsers apply so an empty column's class and tz match a populated one exactly. The three schema-agnostic flattening primitives (`as_dt_row()`, `as_dt_list()`, `parse_delta_ledger()`) have no fixed column set and keep their bare empty; the discriminated-ledger endpoint parsers contribute only their always-present lead columns. A new `test-empty-constructors.R` guard auto-discovers every constructor and asserts none is ever column-less.
